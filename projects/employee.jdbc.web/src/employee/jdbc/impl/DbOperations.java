...
import employee.jdbc.beans.Department;
	...
	*/
	...
	public static ArrayList<Employee> findEmployeeByLastName(DataSource ds, String lastName) throws SQLException{
		...
		try {
			...
			String sqlCmd = "SELECT D.DEPTNAME, E.BIRTHDATE, E.BONUS, E.COMM, E.EDLEVEL, E.EMPNO, E.FIRSTNME, E.HIREDATE, E.JOB, E.LASTNAME, E.MIDINIT, E.PHONENO, "
					+ "E.SALARY, E.SEX, E.WORKDEPT FROM EMP AS E LEFT JOIN DEPT AS D ON E.WORKDEPT = D.DEPTNO "
	...
	/**
	 * Uses JTA to commit a new employee to the database.
	 * 
	 * This method is called when a user presses 'Add employee' button
	 * if the JTA toggle is set to true.
	 * 
	 * It will add the employee based on the values provided in the already
	 * populated bean
	 * 
	...
	public static void createEmployeeWithJta(DataSource ds, Employee employee) throws Exception {
		...
		try {
			...
			// Update a TSQ, within the transaction scope
			TSQ tsq = new TSQ();
	...
	/**
	 * Uses connection.commit to commit a new employee to the database.
	 * 
	 * This method is called when a user presses 'Add employee' button
	 * if the JTA toggle is set to false.
	 * 
	 * It will add the employee based on the values provided in the already
	 * populated bean
	 * 
	 ...
	 * @throws Exception
	 */
	public static void createEmployee(DataSource ds, Employee employee, boolean useJta) throws Exception {
		
		// If we want to use JTA, call the appropriate method
		if(useJta) {
			createEmployeeWithJta(ds, employee);
			return;
		}
	...
	public static void updateEmployeeWithJta(Employee employee, DataSource ds) throws Exception {
		...
		try {
			...
			// Format the template command to a usable format and execute
	...
	private static Employee createEmployeeBean(ResultSet currentResult) throws SQLException{
		...
		// Try to get the department information of the employee
		String departmentNumber = currentResult.getString("WORKDEPT");
		if(departmentNumber == null) {
			employee.setDepartment(null);
		} else {
			employee.setDepartment(new Department(currentResult.getString(1)));
		}
	...
	/**
	 * Populates a CREATE statement with real values, taken
	 * from a provided employee bean.
	 * 
	...
	private static PreparedStatement populateStatement(Employee employee, PreparedStatement statement) throws SQLException {
		...
		// Set a null department, we're not usng it for the application
		String deptno = null;
